...
@register_model("dncnn_SD")
class DnCNN_SD(nn.Module):
	...
	def __init__(self, depth=20, n_channels=64, image_channels=2, bias=False, kernel_size=3):
		...
		self.last_layer = nn.Conv2d(in_channels=n_channels, out_channels=1, kernel_size=kernel_size, padding=padding, bias=self.bias)
	...
	def add_args(parser):
		...
		parser.add_argument("--in-channels", type=int, default=2, help="number of channels")
	...
	def forward(self, x):
		...
		return torch.unsqueeze(y[:, 0, :,:],1)-out
