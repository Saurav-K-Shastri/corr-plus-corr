...
@register_model("dncnn_SD_complex")
class DnCNN_SD_complex(nn.Module):
	...
	def __init__(self, depth=20, n_channels=64, image_channels=3, bias=False, kernel_size=3):
		super(DnCNN_SD_complex, self).__init__()
		kernel_size = 3
		padding = 1
		...
		self.last_layer = nn.Conv2d(in_channels=n_channels, out_channels=2, kernel_size=kernel_size, padding=padding, bias=self.bias)
