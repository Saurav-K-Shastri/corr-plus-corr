...
@register_model("dncnn_cn")
class DnCNN_CN(nn.Module):
    """Made a modification to DnCNN (as defined in https://arxiv.org/abs/1608.03981
    reference implementation: https://github.com/SaoYan/DnCNN-PyTorch) so that it can take in a random realization of the correlated noise as extra channel. Note the chnages done to
    skip connection. Also modified the number of output chnnel in last layer"""
    def __init__(self, depth=20, n_channels=64, image_channels=2, bias=False, kernel_size=3):
        super(DnCNN_CN, self).__init__()
        kernel_size = 3
        padding = 1
        ...
        self.first_layer = nn.Conv2d(in_channels=image_channels, out_channels=n_channels,
        ...
        for i in range(self.depth-2):
            self.hidden_layer_list[i] = nn.Conv2d(
                in_channels=n_channels, out_channels=n_channels, kernel_size=kernel_size, padding=padding, bias=self.bias)
        ...
        self.last_layer = nn.Conv2d(in_channels=n_channels, out_channels=1,kernel_size=kernel_size, padding=padding, bias=self.bias)  # modified wrt to regular dncnn
    ...
    def forward(self, x):
        ...
        return torch.unsqueeze(y[:, 0, :,:],1)-out # modified wrt to regular dncnn
