## I used the following python command to run the file: PYTHONPATH=. python 'train_and_data_generation/train_BF_CNN_corr_p_corr_image_denoiser.py' --output-dir 'corr_p_corr_image_denoiser_experiments'
...
#     train_loader, valid_loader, _ = data.build_dataset(args.dataset, args.data_path, batch_size=args.batch_size)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for batch_id, inputs in enumerate(train_bar):
            ...
            noisy_inputs = get_noisy_data_and_noise_with_same_stat(inputs, min_noise = args.min_noise/255., max_noise = args.max_noise/255., wavetype = 'db1',level=1)
        ...
        if epoch % args.valid_interval == 0:
            ...
            for sample_id, sample in enumerate(valid_bar):
                with torch.no_grad():
                    ...
                    noisy_inputs = get_noisy_data_and_noise_with_same_stat(sample, min_noise = args.min_noise/255., max_noise = args.max_noise/255., wavetype = 'db1',level=1)
...
def get_args():
    ...
    # Add model arguments
    parser.add_argument("--model", default="dncnn_cn", help="model architecture")
