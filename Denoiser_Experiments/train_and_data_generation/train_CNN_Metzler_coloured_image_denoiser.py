## I used the following python command to run the file: PYTHONPATH=. python 'train_and_data_generation/train_CNN_Metzler_coloured_image_denoiser.py' --output-dir dncnn_metzler_colored_image_denoiser_experiments --bias
...
#     dataset_train = DenoiseDataset(os.path.join(datadir, 'train.h5'))
    ...
    train_loader = DataLoader(dataset_train, batch_size=args.batch_size, shuffle=True, num_workers=16)
    ...
    for epoch in range(start_epoch, args.num_epochs):
        ...
        for batch_id, inputs in enumerate(train_bar):
            ...
            noisy_inputs, stds = get_noisy_data_and_stds(inputs, min_noise = args.min_noise/255., max_noise = args.max_noise/255., wavetype = 'db1',level=1)
            ...
            with torch.cuda.amp.autocast(enabled=True): # allows half-precision
                    outputs = model(noisy_inputs,stds)
        ...
        if epoch % args.valid_interval == 0:
            ...
            for sample_id, sample in enumerate(valid_bar):
                with torch.no_grad():
                    ...
                    noisy_inputs, stds = get_noisy_data_and_stds(sample, min_noise = args.min_noise/255., max_noise = args.max_noise/255., wavetype = 'db1',level=1)
        
                    sample = sample.to(device)
                    noisy_inputs = noisy_inputs.to(device)
                    stds = stds.to(device)
            
                    output = model(noisy_inputs,stds)
                    ...
                    if writer is not None and sample_id < 10:
                        image = torch.cat([sample, torch.unsqueeze(noisy_inputs[:,0,:,:],1), output], dim=0)
...
def get_args():
    ...
    # Add model arguments
    parser.add_argument("--model", default="dncnn_dvdamp_coloured", help="model architecture")
