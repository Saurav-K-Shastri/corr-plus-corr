...
def mul(w1, w2):
    """Multiplication of two wavelets element-wise (band-by-band).
    ...
    """
    result = w1.copy()
    result.coeff[0]  = w1.coeff[0]*w2.coeff[0] 
    for i in range(1, len(result.coeff)):
        for j in range(len(result.coeff[i])):
            result.coeff[i][j] = w1.coeff[i][j]*w2.coeff[i][j]
    return result
...
def dot_product(w1, w2):
    ...
    """
    result = w1.copy()
    result.coeff[0]  = w1.coeff[0].conj()*w2.coeff[0] 
    for i in range(1, len(result.coeff)):
        for j in range(len(result.coeff[i])):
            result.coeff[i][j] = w1.coeff[i][j].conj()*w2.coeff[i][j]
    return sum(result)

def sum(w1):
    """Sum of all the elements of the wavelet
    ...
    """
    result = 0.0
    result += np.sum(w1.coeff[0])
    for i in range(1, len(w1.coeff)):
        for j in range(len(w1.coeff[i])):
            result += np.sum(w1.coeff[i][j])
    return result
...
def get_noise_subbandwise_for_dncnn_cn(wavelet, stds, is_complex=False):
    """ modified. decreption not accurate
        Add Gaussian noise to each wavelet subband.
    ...
    """
    result = wavelet.copy()
    result_noise_2 = wavelet.copy()
    
    result.coeff[0] += gutil.generate_noise(result.coeff[0].shape, stds[0], ret_array=True, is_complex=is_complex)
    result_noise_2.coeff[0] = gutil.generate_noise(result.coeff[0].shape, stds[0], ret_array=True, is_complex=is_complex)
    
    idx = 1
    for i in range(1, len(result.coeff)):
        for j in range(len(result.coeff[i])):
            result.coeff[i][j] += gutil.generate_noise(result.coeff[i][j].shape, stds[idx], ret_array=True, is_complex=is_complex)
            result_noise_2.coeff[i][j] = gutil.generate_noise(result.coeff[i][j].shape, stds[idx], ret_array=True, is_complex=is_complex)
            idx += 1
#     print(idx)
    return result, result_noise_2


def get_noise_subbandwise_for_dncnn_cn_five(wavelet, stds, is_complex=False):
    """ modified. decreption not accurate
        Add Gaussian noise to each wavelet subband.
    ...
    """
    result = wavelet.copy()
    result_noise_1 = wavelet.copy()
    result_noise_2 = wavelet.copy()
    result_noise_3 = wavelet.copy()
    result_noise_4 = wavelet.copy()
    result_noise_5 = wavelet.copy()
    
    result.coeff[0] += gutil.generate_noise(result.coeff[0].shape, stds[0], ret_array=True, is_complex=is_complex)
    result_noise_1.coeff[0] = gutil.generate_noise(result.coeff[0].shape, stds[0], ret_array=True, is_complex=is_complex)
    result_noise_2.coeff[0] = gutil.generate_noise(result.coeff[0].shape, stds[0], ret_array=True, is_complex=is_complex)
    result_noise_3.coeff[0] = gutil.generate_noise(result.coeff[0].shape, stds[0], ret_array=True, is_complex=is_complex)
    result_noise_4.coeff[0] = gutil.generate_noise(result.coeff[0].shape, stds[0], ret_array=True, is_complex=is_complex)
    result_noise_5.coeff[0] = gutil.generate_noise(result.coeff[0].shape, stds[0], ret_array=True, is_complex=is_complex)
    ...
    for i in range(1, len(result.coeff)):
        for j in range(len(result.coeff[i])):
            result.coeff[i][j] += gutil.generate_noise(result.coeff[i][j].shape, stds[idx], ret_array=True, is_complex=is_complex)
            result_noise_1.coeff[i][j] = gutil.generate_noise(result.coeff[i][j].shape, stds[idx], ret_array=True, is_complex=is_complex)
            result_noise_2.coeff[i][j] = gutil.generate_noise(result.coeff[i][j].shape, stds[idx], ret_array=True, is_complex=is_complex)
            result_noise_3.coeff[i][j] = gutil.generate_noise(result.coeff[i][j].shape, stds[idx], ret_array=True, is_complex=is_complex)
            result_noise_4.coeff[i][j] = gutil.generate_noise(result.coeff[i][j].shape, stds[idx], ret_array=True, is_complex=is_complex)
            result_noise_5.coeff[i][j] = gutil.generate_noise(result.coeff[i][j].shape, stds[idx], ret_array=True, is_complex=is_complex)
            idx += 1
    return result, result_noise_1,  result_noise_2,  result_noise_3,  result_noise_4,  result_noise_5
...
def mul_subbandwise_scalar(wavelet, scalar):
    """Multiply each subband of a wavelet by a scalar.
    ...
    """
    result = wavelet.copy()
    result.coeff[0] *= scalar
    for i in range(1, len(result.coeff)):
        for j in range(len(result.coeff[i])):
            result.coeff[i][j] *= scalar
    return result