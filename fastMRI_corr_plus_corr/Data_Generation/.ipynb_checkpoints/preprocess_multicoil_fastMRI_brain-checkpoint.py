...
# cmd to run: PYTHONPATH=. python 'preprocess_multicoil_fastMRI_brain.py' 
...
from data_fastMRI.mri_data_new import SelectiveSliceData_Train_Brain
from data_fastMRI.mri_data_new import SelectiveSliceData_Val_Brain
import scipy.misc
import copy
import pathlib
...
def create_test_datasets(args):
    ...
    train_data = SelectiveSliceData_Train_Brain(
    ...
    val_data = SelectiveSliceData_Val_Brain(
        root=args.data_path_val,
        transform=DataTransform(args),
        challenge='multicoil',
        sample_rate=1,
        use_top_slices=True,
        number_of_top_slices=4, 
        fat_supress=None,
        strength_3T=None,
        restrict_size=False,
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data-path-train", default='/storage/fastMRI/data/multicoil_brain/train', help="path to data directory for training") # change this based on your data directory
    parser.add_argument("--data-path-val", default='/storage/fastMRI/data/multicoil_brain/val', help="path to data directory for validation") # change this based on your data directory
    parser.add_argument("--data-save-path", default='/storage/fastMRI/data/multicoil_brain/pre_processed/', help="path to data directory")# change this based on where you want to save the files
